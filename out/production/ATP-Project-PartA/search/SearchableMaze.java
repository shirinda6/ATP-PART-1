    ...
    public ArrayList<AState> getAllPossibleStates(AState current) {
        ...
        for (int i=-1; i<2; i++){
            ...
            if (position.getColumnIndex() + i >= 0) {
                if (m1[position.getRowIndex()][position.getColumnIndex() + i] == 1)
            ...
            if (position.getRowIndex() + i >= 0) {
                if (m1[position.getRowIndex() + i][position.getColumnIndex()] == 1)
            ...
            for (int j = -1; j < 2; j++) {
                if (j==0)
                    continue;
                if (position.getRowIndex() + i >= 0 && position.getColumnIndex() + j >= 0) {
                    if (m1[position.getRowIndex() + i][position.getColumnIndex() + j] == 1)
